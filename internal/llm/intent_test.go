...
12. 对“操作电脑”“帮我在桌面放一个文件”“帮我做个网页并保存下来”“帮我整理一个文档”这类请求，只要需要本机执行和产出物，就优先视为 complex_task。
13. 如果用户是在补充、修改、继续之前已经完成的任务，例如“刚刚那个网页再加一个按钮”“把上次那个文件改一下”“在刚才那个任务基础上继续做”，优先调用 continue_task。
14. 任务链摘要已经按时间整理出：初始任务需求、中间轮次对话、任务最后回答。判断 continue_task 时，要结合整段摘要一起理解，不要只看某一句。
15. 调用 continue_task 时，只需要提供 task_id 和 request 两个字段。
16. 如果任务链摘要里给出了 latest_task_id，那么 task_id 必须填写对应摘要里的 latest_task_id，不要编造，也不要回退到更早的任务 ID。
17. 如果用户这次更像是在追一个仍在执行中的任务状态，而不是继续一个已经完成的任务，不要调用 continue_task，优先考虑 query_task_progress。
`)
...
func TestBuildIntentMessages(t *testing.T) {
	...
	completedTasks := "下面是最近可继续的任务链摘要。每条摘要都代表一条任务链当前最新的已完成节点。\n\n[latest_task_id=task_3]\n初始任务需求：帮我做一个关于天气的小游戏\n中间轮次对话：\n- 任务执行器：第一版小游戏已经完成。\n- 用户追加输入：加一点动画\n- 任务执行器：已经补上基础动画。\n任务最后回答：已经补上基础动画。"
