...
它关注的是整体结构、状态流转、任务结果汇报和任务续做这条主线，不展开具体执行器的接入细节。像 Claude Code 这种执行器的接入方式，会放在独立文档里说明。
...
4. 任务完成后，系统不会粗暴抢话；会在语音通道空闲时主动做任务结果汇报，若当前忙则延后到安全时机。
...
5. 任务有结果后，不会立刻抢话；如果当前语音通道空闲，会主动做任务结果汇报，否则会延后到后续安全时机。
...
- `result`：保存最终结果文本
- `result_report_pending`：表示结果是否还没向用户汇报
