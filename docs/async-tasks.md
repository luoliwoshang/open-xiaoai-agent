...
- reply model 拿到的不是只看“最终 delivery 状态”；
- 它还会尽量知道“本轮刚刚发生了什么发送动作”。
...
当前系统已经把这两件事拆开建模了，并且当前这期已经实现：

- 任务结果汇报前，按当前默认 IM 渠道即时尝试一次产物发送
- 没有可用渠道时，明确告诉用户当前发不出去
- 用户后续重新绑定默认渠道后，对旧产物自动补发
- 如果当前还有可用语音通道，补发结果也会再交给 reply model 润色后播报给用户
...
除了“任务刚完成时的首次结果汇报”，当前还支持另一条链路：

- 某个任务当时已经完成
- 但完成时还没有可用默认 IM 渠道
- 对应 artifact delivery 被记成 `no_channel`
- 之后用户重新保存了默认 IM 渠道配置
...
1. 当前有最近一次可复用的语音通道：
   - 会先补发图片 / 文件
   - 再把“这不是新的任务完成通知，而是之前没送达的产物刚刚重新投递了”这类结构化上下文交给 reply model
   - 最后再播报一条自然的补投递通知
2. 当前没有任何可用语音通道：
   - 仍然会先静默补发文件
   - 只是暂时不播语音提示

也就是说，“补发成功后要不要说出来”这件事，仍然遵守当前主流程的语音通道边界：

- 有通道，就润色后播报
- 没通道，就先完成文件补发，不强行制造一条不存在的语音会话
...
sequenceDiagram
    ...
    rect rgb(245, 248, 255)
        ...
        alt 当前任务没有产物
            ...
            Agent->>Reply: 传任务结果 + “本轮没有可用渠道发送产物”
        else 当前任务有产物且已有默认渠道
            loop 每个待发送产物
                Agent->>IM: 发送图片 / 文件
                alt 发送成功
    ...
    rect rgb(248, 250, 245)
        note over Settings,Voice: 历史产物补投递
        Settings-->>Agent: 默认 IM 渠道已保存
        Agent->>Tasks: 读取任务链最新节点里的未送达产物
        loop 每个待补发产物
            Agent->>IM: 重新发送图片 / 文件
            alt 发送成功
                Agent->>Tasks: 标记 delivered
        ...
        alt 当前仍有可用语音通道
            Agent->>Reply: 传“历史产物补投递通知”上下文
            Reply-->>Agent: 返回适合播报的话术
            Agent-->>Voice: 播放补投递通知
        else 当前没有可用语音通道
            Note over Agent: 只静默完成文件补发，不强行播报
        end
    end
```
...
- 如果用户之后才把默认渠道配好，之前没发出去的文件也不会一直滞留，而是会被重新补发。
- 如果补发时当前还有可用语音通道，系统会再自然提醒一句“之前那个结果我刚刚已经重新发到微信了”，而不是只在后台默默补一遍。
...
- 当前没有默认 IM 渠道时，会先明确告知“当前没有可用的渠道发送产物”；等用户后续把默认渠道配好后，系统会对历史未送达产物做自动补发
