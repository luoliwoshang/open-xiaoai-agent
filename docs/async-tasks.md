...
4. 任务完成后，系统不会粗暴抢话；会在语音通道空闲时主动补报，若当前忙则延后到安全时机。
...
5. 任务有结果后，不会立刻抢话；如果当前语音通道空闲，会主动补报，否则会延后到后续安全时机。
...
sequenceDiagram
    ...
    alt 当前语音通道空闲
        Tasks-->>Agent: 通知有待补报结果
        Agent->>Tasks: 读取待补报任务结果
        Agent-->>Device: 主动补报任务新进展
    else 当前还有语音主流程在执行
        Tasks-->>Agent: 通知有待补报结果
        Agent->>Tasks: 保留 pending 标记
        User->>Device: 下一次正常对话
        Device->>Agent: 转发新的 ASR 文本
        Agent->>Tasks: 在当前轮结束后读取待补报任务结果
...
2. 标记这条结果“还没有向用户补报”。
3. 如果当前语音通道空闲，就主动补报。
4. 如果当前还有主流程正在播报或处理，就先保留 pending 标记，等当前轮结束后再补报。

这个选择很重要，因为它把“后台任务完成”从系统事件，转换成了“对用户体验友好的后续通知”。

为了方便排障，Dashboard 还会暴露主语音通道的运行时状态。当前至少会展示：

- 是否正在处理一轮会发声的主流程
- 是否已有待补报结果 ready，但因为语音通道忙而暂未播报
- 是否已经保留了一个可用于主动播报的最近 session
...
3. 任务系统主动通知主流程：现在有一条结果可以尝试补报了。
   这里不是靠定时轮询数据库，而是任务状态刚变化时触发一次事件通知。

4. 主流程判断当前语音通道是否空闲。
   - 如果当前没有别的会发声流程在运行，就立即开始补报。
   - 如果当前还在处理别的对话或播报，就先保留 `report_pending`，等当前轮结束后再尝试。

5. 真正补报时，不会机械地念任务标题，而是把待补报任务整理成一段结构化上下文，再交给回复模型生成适合 TTS 的话术。

6. 只有这段补报真正播放成功后，系统才会把对应任务标记为“已汇报”，也就是清掉 `report_pending`。
...
- 如果补报生成失败、播放失败，`report_pending` 不会被清掉，后续仍然有机会再次补报。
...
所以当前做法不是“任务一结束就强行插话”，而是“任务一结束先记下来；如果当前空闲就主动补报，如果当前忙就等安全时机再补报”。
...
- 任务补报不会主动抢占当前语音通道；如果系统正在处理别的语音主流程，就会延后补报
