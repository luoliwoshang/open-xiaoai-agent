...
当前 `intent` 已经会读取当前 `historyKey` 对应的长期记忆。

接入方式和 reply 一样，都是：

- 先按当前 `historyKey` 召回长期记忆
- 再把这段记忆包装成一条 system message
- 然后和最近会话窗口一起交给 intent 模型

这样做的原因是：

- 有些工具相关背景本来就只存在长期记忆里
- 例如 Home Assistant 的地址、已有系统、家庭环境、常用服务名
- 如果 intent 看不到这些信息，就可能误判“没有这个系统”或走错工具

这里仍然保留同样的约束：

- 长期记忆只是辅助上下文
- 不要把它伪装成用户刚刚这一轮新输入
- 如果长期记忆为空，就不会额外插入这条 system message
